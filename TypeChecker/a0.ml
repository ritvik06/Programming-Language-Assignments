...
let negate (s1,l1) : (bigint) = match s1 with
	Neg -> (NonNeg,l1)
	|NonNeg -> (Neg,l1);;

let negBool (a:bool) : bool = match a with
true -> false
|false -> true;;

let ifte (a:bool) (b:bigint) (c:bigint) :bigint = (if(a=true) then b else c);;

let conjunction bool1 bool2 = match bool1 with
false -> false
| true -> (match bool2 with
			true -> true
			| false -> false)
| _ -> raise (Invalid_Input);;

let disjunction bool1 bool2 = match bool1 with
true -> true
| false -> (match bool2 with
			true -> true
			| false -> false)
| _ -> raise (Invalid_Input);;		
...
let rec gt ((s1,l1): bigint) ((s2,l2): bigint)  : bool =  let l1 = (trim_zeroes l1) and l2 = (trim_zeroes l2) in  match (s1,l1) with 
	...
	|( Neg, x::xs) -> (match (s2,l2) with
				( NonNeg, y::ys ) -> false
				| (Neg, y::ys) -> if (l1=l2) then false else (if (length l1) > (length l2) then false else if (length l1) < (length l2) then true 
				else (if x<y then true else if x>y then false else (gt (Neg,xs) (Neg,ys)) ) )
				| (_,_) -> false)
	|( NonNeg, x::xs) -> (match (s2,l2) with
				( Neg, y::ys ) -> true
				| (NonNeg, y::ys) -> if(l1=l2) then false else (if (length l1) > (length l2) then true else if (length l1) < (length l2) then false
				else (if x<y then false else if x>y then true else (gt (NonNeg,xs) (NonNeg,ys))) )
				| (_,_) -> false );;
...
(*Code Submitted by Shreya Sharma-allowed by the instructor to be used for assignment 1*)